...
// GetGroupSig retrieves the block groupSig corresponding to the
// hash.
func GetGroupSig(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64, header *types.Header) (types.GroupSignatures, error) {
	sig := &GroupSigRequest{Hash: hash, Number: number, Header: header}
	if err := odr.Retrieve(ctx, sig); err != nil {
		return nil, err
	}
	return sig.GroupSig, nil
}
...
func GetBlock(ctx context.Context, odr OdrBackend, hash common.Hash, number uint64) (*types.Block, error) {
	...
	return types.NewBlockWithHeader(header).WithBody(body.Transactions, body.Uncles, body.GroupSig), nil
}
