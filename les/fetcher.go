...
func (f *lightFetcher) registerPeer(p *peer) {
	p.lock.Lock()
	p.hasBlock = func(hash common.Hash, number uint64, wantLock bool) bool {
		return f.peerHasBlock(p, hash, number, wantLock)
	}
	p.lock.Unlock()
...
func (f *lightFetcher) peerHasBlock(p *peer, hash common.Hash, number uint64, wantLock bool) bool {
	if wantLock {
		f.lock.Lock()
	}
	defer func() {
